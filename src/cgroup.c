...
static int init_cpuset_cgroup(pid_t, char *);
static int init_cpu_cgroup(pid_t, char *);
static int init_memory_cgroup(pid_t, char *);
static int init_freezer_cgroup(pid_t, char *);
static int cgroup_mkdir_name(int, char *);
static int cgroup_fd_operation(int, char *, char *, char *);
...
static int init_cpu_cgroup(pid_t pid, char * name){
    char buffer[100];
...
static int init_cpuset_cgroup(pid_t pid, char * name){
...
static int init_memory_cgroup(pid_t pid, char * name){
...
static int init_freezer_cgroup(pid_t pid, char * name){
    char buffer[100];
...
static int cgroup_mkdir_name(int type, char * container_name){
    char dirpath[100];
    snprintf(dirpath, sizeof(dirpath), "/sys/fs/cgroup/%s/%s", CgroupType[type], container_name);
    return mkdir(dirpath,0755);
}

static int cgroup_fd_operation(int type, char * container_name, char * filename, char *towrite){
    char filepath[100];
